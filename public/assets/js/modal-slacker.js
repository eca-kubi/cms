...
var employees_leave_status_table;
...
domReady = function () {
  ...
  // add a back button
  /* $('.box-title').prepend('<a class="btn w3-btn bg-gray w3-small mx-1" href="javascript:history.go(-1)" role="button"><i class="fa fa-arrow-alt-circle-left"></i> Go back</a>'); */
  ...
  form = $('form[data-toggle=validator]').validator({ delay: '100000000' }).on('submit', function (e) {
    if ($('input[name=personal_email]').val().trim() != '') {
      $('input[name=company_email]').removeAttr('required')
      //form.validator('update')
      form.validator('validate')
    } else {
      $('input[name=company_email]').attr('required', true)
      //form.validator('update')
      form.validator('validate')
    }
  ...
  leaveapplicants_mgr_table = $('#leaveapplicants_mgr_table').DataTable({
    autoWidth: true,
    "dom": "<'row controls'<'col-sm-12 col-md-4'l><'col-sm-12 col-md-4'f>>" +
      "<'row'<'col-sm-12'tr>>" +
      "<'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7'p>>",
    /* buttons: [{
  ...
  employees_leave_status_table = $('#employees_leave_status_table').DataTable({
    autoWidth: true,
    /* columnDefs: [
      {
        "width": "10%", "targets": 0,
...
window.addEventListener("load", function (event) {
  setTimeout(() => {
    ...
    $('footer').removeClass('d-none');
    setTimeout(function() {
       $('body').scrollTo('.box', 1000, { offset: -120 });
    }, 1000);
  }, 500);
